...
$(document).ready(function () {
	...
	//$()
	$('#referralId').keyup(function(e){
		//console.info("manish");
		if(e.keyCode == 13)
		{
			var id = $(this).val();
			var x = getCustomerInfoForReferral(id);
		}	
	});
	
});

var getCustomerInfoForReferral = function(id){
		...
		$.ajax({
			...
			success : function(data) {
				if(validateEmail(data.emailId)){
					document.getElementById('eligibilty-marker').innerHTML = "(Eligible)";
					console.info("true");
					document.getElementById('eligibilty-marker').style.color = "green";
					notEligible = false;
				}
				else{
					document.getElementById('eligibilty-marker').innerHTML = "(Not Eligible)";
					document.getElementById('eligibilty-marker').style.color = "red";
					notEligible = true;
...
var moveToZeroState = function(){
	 issue_amt = 0;
	 amt1 = 0;
	 threshold = 0;
	 amt2 = 0;
	 validity = 0;
     hasGiftCard = 0;  //No gift card
	 remainingBalanace = 0;
     GiftCardDto = new Object();
     moveToZeroStateCustomerJS();
}