...
var addProduct = function(element) {
	...
	if (value != "0") {
		parameter = "\"" + value + "\"";
		$('#sport-filter option[value=' + parameter + ']').prop('selected',
				'selected').change();
	} else {
		$('#sport-filter option[value="other"]').prop('selected', 'selected')
				.change();
	}
...
var clearFeedbackModal = function() {
	document.getElementById('cid').value = '';
	document.getElementById('cphone').value = '';
	document.getElementById('amazon-checkbox').checked = false;
	document.getElementById('ebay-checkbox').checked = false;
...
var retrieveProduct = function(element) {
	...
	// "http://ecx.images-amazon.com/images/I/31LxLxHO-CL._AA160_FMwebp_QL70_.jpg");
	...
	var t1 = document.createTextNode("Details");
	btn1.appendChild(t1);
	btn1.setAttribute("data-target", "#product-add-modal");
...
var displayProductInGrid = function(evt) {
	...
	$.ajax({
		url : 'getAllProduct',
		data : {
			'sport_id' : $("#sport-type-filter").val(),
			'brand' : brand,
			'textSearchInput' : textSearch
		},
		success : function(data) {
...
var distributeProduct = function(element) {
	...
	console.info("pid at bottom =" + element.productId + " " + element.colors
			+ " " + element.sizes);
